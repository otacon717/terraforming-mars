...
import { ALL_VENUS_CORPORATIONS, ALL_PRELUDE_CORPORATIONS, ALL_CORPORATION_CARDS, ALL_COLONIES_CORPORATIONS, ALL_TURMOIL_CORPORATIONS, ALL_PROMO_CORPORATIONS, ALL_CORP_ERA_CORPORATION_CARDS, ALL_REPLACEMENT_CORPORATIONS } from '../Dealer';
...
const allItems: Array<CardName> = [
	...
	...ALL_REPLACEMENT_CORPORATIONS.map((cf) => cf.cardName)
];

export const CorporationsFilter = Vue.component("corporations-filter", {
    props: ["corporateEra", "prelude", "venusNext", "colonies", "turmoil", "promoCardsOption", "HandicapOption"],
    data: function () {
        return {
            ...
            selectedCorporations: [
				...
				...this.HandicapOption ? ALL_REPLACEMENT_CORPORATIONS.map((cf) => cf.cardName) : []
            ],
            corporationGroups: [
                ...
                {"title": CorporationGroup.HANDICAP, "items": ALL_REPLACEMENT_CORPORATIONS.map((cf) => cf.cardName)}
            ]
        }
    },
    methods: {
    ...
    watch: {
		...
		HandicapOption: function (enabled) {
            enabled ? this.selectAll(CorporationGroup.HANDICAP) : this.selectNone(CorporationGroup.HANDICAP);
        }
    },
    template: `
